...
class TestSandboxFunctions:
    ...
    def test_allow_read_dir(self):
        ...
        paths = [Path("/tmp")]
        with (
            patch("coding_assistant.sandbox.Path.exists", return_value=True),
    ...
    def test_allow_read_file(self):
        ...
        with (
            ...
            coding_assistant.sandbox.allow_read(mock_rs, paths)
            mock_rs.allow.assert_called_with(
                Path("/tmp/test"), rules=coding_assistant.sandbox._get_read_only_file_rule()
            )
