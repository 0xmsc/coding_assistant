...
class RemoteWorkerRunner:
    ...
    async def run_prompt(
        ...
        *,
        ...
        commit_future: asyncio.Future[RemoteCommit] = asyncio.get_running_loop().create_future()

        async def handle_event(event: RemoteClientEvent) -> None:
            if isinstance(event, RemotePromptFailedEvent) and not commit_future.done():
                commit_future.set_exception(RemoteWorkerError(event.message))

        async def handle_disconnect(endpoint: str) -> None:
            if not commit_future.done():
                commit_future.set_exception(RemoteWorkerError(f"Remote worker {endpoint} disconnected."))

        async def handle_commit(commit: RemoteCommit) -> None:
            if commit.session_id != prompt.session_id:
                return
            if commit.base_version != prompt.base_version:
                if not commit_future.done():
                    commit_future.set_exception(
                        RemoteWorkerError(
                            f"Worker commit for {prompt.session_id} used base version "
                            f"{commit.base_version}, not {prompt.base_version}.",
                        ),
                    )
                return
            if not commit_future.done():
                commit_future.set_result(commit)
