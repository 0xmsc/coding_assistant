...
@pytest.mark.asyncio
async def test_shell_execute_timeout(execute: Any) -> None:
...
@pytest.mark.asyncio
async def test_shell_execute_nonzero_exit_code(execute: Any) -> None:
    out = await execute.fn(command="bash -lc 'exit 7'")
    assert out.startswith("Exit code: 7.\n\n")


@pytest.mark.asyncio
async def test_shell_execute_truncates_output(execute: Any) -> None:
...
@pytest.mark.asyncio
async def test_shell_execute_happy_path_stdout(execute: Any) -> None:
    out = await execute.fn(command="printf 'hello'", timeout=5)
    assert out == "hello"


@pytest.mark.asyncio
async def test_shell_execute_stderr_captured_with_zero_exit(execute: Any) -> None:
    out = await execute.fn(command="echo 'oops' >&2; true", timeout=5)
    assert out == "oops\n"


@pytest.mark.asyncio
async def test_shell_execute_nonzero_with_stderr_content(execute: Any) -> None:
...
@pytest.mark.asyncio
async def test_shell_execute_echo(execute: Any) -> None:
