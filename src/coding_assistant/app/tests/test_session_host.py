...
async def test_session_host_runs_local_prompt_and_updates_history() -> None:
    session_host = SessionHost(
        ...
        model="test-model",
        tools=[],
        streamer=ScriptedStreamer([AssistantMessage(content="Hello from the worker")]),
    )
    ...
    async with session_host.subscribe() as queue:
        ...
        result = await session_host.submit_local_prompt("Hi")
...
async def test_session_host_rejects_local_prompt_when_remote_controller_is_connected() -> None:
    ...
    assert await session_host.attach_remote_controller() is True

    result = await session_host.submit_local_prompt("Hi")

    assert result.accepted is False
    assert result.reason == "remote_connected"
...
async def test_session_host_rejects_new_prompt_while_run_is_in_flight() -> None:
    ...
    first_result = await session_host.submit_local_prompt("Hi")
    assert first_result.accepted is True

    second_result = await session_host.submit_remote_prompt("Again")
...
async def test_session_host_remote_disconnect_cancels_run_and_restores_local_input() -> None:
    ...
    async with session_host.subscribe() as queue:
        ...
        prompt_result = await session_host.submit_remote_prompt("Do the task")
