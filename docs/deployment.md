...
Run exactly one manager against a given `/data/sessions.sqlite`. Multiple
managers sharing a database are unsupported. Within the manager, only one
worker may be active for each session.
