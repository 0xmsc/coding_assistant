...
- Fix MCP server lifecycle management. `MCPServerManager` has an unused `_exit_stack`, and `start()` stores a `Client` while returning from inside `async with client`, which likely leaves the stored client closed before later `mcp_call` usage. Either use the exit stack to keep clients open until `stop()`/`close()`, or simplify the manager around one-shot calls.

## 2. Security Findings To Improve
...
- MCP success paths are not covered. Existing MCP manager tests cover config validation and not-running errors, but not successful `start()`, `list_tools()`, `call()`, or `close()` behavior with a fake FastMCP client. This is also where the likely client-lifecycle bug should be caught.
